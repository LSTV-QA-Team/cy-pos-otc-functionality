...
Cypress.Commands.add("login", (userCode, userPassword) => {
  ...
  cy.get(
    'span[role="img"][aria-label="close"][tabindex="-1"].anticon.anticon-close'
  ).click();
  cy.wait(4000);
...
Cypress.Commands.add(
  ...
  (
    ...
    cy.get("table.MuiTable-root thead tr")
      ...
      .should("have.length", expectedColTitle.length)
      .then(($headers) => {
        const errorMessages = [];
        ...
        $headers.each((index, element) => {
          elementPromises.push(
            new Cypress.Promise((resolve) => {
              const headerText = Cypress.$(element)
                .find(".Mui-TableHeadCell-Content-Wrapper")
                .text();
              ...
              if (assertionPassed) {
                ...
                if (!failedAssertions.has(customErrorMessage)) {
                  ...
                  cy.screenshot(
                    `failure-${index}-${expectedColTitle[index].replace(
                      /\W/g,
                      "-"
                    )}`,
                    { capture: "fullPage" }
      ...
      .then(($element) => {
        ...
        if (containsExpectedText) {
          ...
          if (!failedAssertions.has(failureMessage)) {
            ...
            cy.screenshot(
              `failure-${failureMessage}-${selector.replace(/\W/g, "-")}`,
              { capture: "fullPage" }
            );
          } else {
            cy.log("Skipping screenshot, failure already captured.");
...
Cypress.Commands.add(
  "checkForFailure",
  (assertionResults, failureMessages = []) => {
    cy.fixture("message.json").then((data) => {
      ...
      if (hasFailedAssertions || hasFailedJson) {
        ...
        const consolidatedErrors = failureMessages.reduce(
          (acc, { referenceNumber, errorContext, message }) => {
        ...
        const errorLog = Object.entries(consolidatedErrors)
          .map(([reference, { context, messages }]) => {
          ...
          return cy.get(element.sel).then(($el) => {
            ...
            if (assertionPassed) {
              ...
              if (!failedAssertions.has(customErrorMessage)) {
                ...
                cy.screenshot(
                  `failure-${failureMessage}-${element.sel.replace(
                    /\W/g,
                    "-"
                  )}`,
                  { capture: "fullPage" }
      ...
      .then(($body) => {
        ...
        if ($body.find(selector).length > 0) {
          ...
          if (!failedAssertions.has(failureMessage)) {
            ...
            cy.screenshot(`failure-${selector.replace(/\W/g, "-")}`, {
              capture: "fullPage",
            });
          } else {
            cy.log("Skipping screenshot, failure already captured.");
