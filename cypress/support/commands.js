      ...
      .then(($element) => {
        ...
        if (containsExpectedText) {
          ...
          if (!failedAssertions.has(failureMessage)) {
            ...
            cy.screenshot(`failure-Reference No.:${referenceNumber}`, {
              capture: "fullPage",
            });
          } else {
          ...
          return cy.get(element.sel).then(($el) => {
            ...
            if (!processedScenarios.has(referenceNumber)) {
              ...
              assertionResults.push({
                data: assertionPassed ? "passed" : "failed",
              });
            }
          });
        });
    ...
    cy.get("table.MuiTable-root thead tr")
      ...
      .then(($headers) => {
        ...
        if ($headers.length === expectedColTitle.length) {
          ...
          const customErrorMessage = `Expected ${
            expectedColTitle.length
          } columns "${expectedColTitle.join(", ")}" but found ${
            $headers.length
          } "${actualColTitles.join(", ")}"`;
          cy.log(
            "Assertion Failed: Number of headers does not match expected length"
          );
          failureMessages.push({
            referenceNumber,
            errorContext,
