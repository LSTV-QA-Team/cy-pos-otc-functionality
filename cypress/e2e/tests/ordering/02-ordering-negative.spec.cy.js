...
describe.only("Ordering ", () => {
  ...
  //verify Header title
    cy.contains(/^Cashiering$/).should('be.visible')

  // Verify if the target button is disabled or no disable
    cy.get('.sc-gtLWhw').should('not.be.disabled'); // Cash Fund
    cy.get(':nth-child(2) > .sc-ivxoEo').should('be.disabled'); // Cash In
    cy.get(':nth-child(3) > .sc-ivxoEo').should('be.disabled'); // Cash Out
    cy.get(':nth-child(4) > .sc-ivxoEo').should('be.disabled'); // Cash Declaration
...
// FREE ITEM (Negative Testing)
  ...
  it("Price Override", () => {
    ...
    cy.get('#input-container > .undefined').click().clear().type('1000',{force:true})
    cy.get('.border-green-500').click()
    cy.get('.Toastify__toast-body')
  ...
  it("Discount", () => {
    ...
    cy.get('.border-green-500').click({force:true}).wait(500)
  ...
  it("Reprint Void Transaction", () => {
    ...
    cy.checkText('section > .overflow-y-auto > :nth-child(1) > :nth-child(2)', '0', 'Reprint Void Transaction:', '82.79', assertionResults, failureMessages)  
    cy.get('.px-8 > .flex > .anticon > svg').click()
    cy.wait(1000)
    cy.checkForFailure(assertionResults, failureMessages)
  })
  ...
  it("Reprint Refund Transaction", () => {
    ...
    cy.checkText('#root > div:nth-child(2) > div.absolute.w-full.h-full.z-20 > div > div > div > div.max-h-\\[500px\\].min-w-\\[500px\\].max-w-\\[600px\\].my-5.px-10.font-montserrat.overflow-y-auto.overflow-x-hidden.relative.w-auto > section > div.flex.flex-col.overflow-y-auto.h-\\[500px\\].px-2 > div', '0', 'Reprint Refund:', 'INV-000000000000000282.79', assertionResults, failureMessages)
